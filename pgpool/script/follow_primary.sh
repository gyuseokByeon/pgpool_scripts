...
ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${SSH_USER}@${NEW_PRIMARY_NODE_HOST} -i ~/.ssh/id_rsa_pgpool ls /tmp > /dev/null

if [ $? -ne 0 ]; then
    echo follow_main.sh: passwordless SSH to ${SSH_USER}@${NEW_PRIMARY_NODE_HOST} failed. Please setup passwordless SSH.
    exit 1
fi
...
if [ $PGVERSION -ge 12 ]; then
    ...
    RECOVERYCONF=${NODE_PGDATA}/postgresql.auto.conf
else
...
# ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    ${SSH_USER}@${NODE_HOST} -i ~/.ssh/id_rsa_pgpool ${PGHOME}/bin/pg_ctl -w -D ${NODE_PGDATA} status
...
# ${PGHOME}/bin/pg_ctl -l /dev/null -w -D ${NODE_PGDATA} start
    ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${SSH_USER}@${NODE_HOST} -i ~/.ssh/id_rsa_pgpool "
...
EOT
        ...
        ssh -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
            ${SSH_USER}@${NODE_HOST} -i ~/.ssh/id_rsa_pgpool sudo systemctl start postgresql-13
            # ${SSH_USER}@${NODE_HOST} -i ~/.ssh/id_rsa_pgpool $PGHOME/bin/pg_ctl -l /dev/null -w -D ${NODE_PGDATA} start
